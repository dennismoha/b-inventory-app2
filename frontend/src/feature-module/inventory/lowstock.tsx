...
import type { LowStockResponseItem } from '../interface/features-interface';
import { getDefaultMRTOptions } from '@components/material-react-data-table';

const defaultMRTOptions = getDefaultMRTOptions<LowStockResponseItem>();

export default function StockTable() {
  const { data } = useFetchLowStockQuery();
  // const [updateStockLevel] = useUpdateStockLevelMutation();

  const stockData = data?.data ?? [];
  console.log('stock data ', stockData);

  const columns = useMemo<MRT_ColumnDef<LowStockResponseItem>[]>(
    () => [
      {
        accessorKey: 'supplier_products_id',
        header: 'suppplier products'
      },
      {
      ...
      {
        header: 'In Stock',
        accessorFn: (row) => row.total_received - row.total_sold
      },
      {
  ...
  //         supplier_products_id: values.supplier_products_id,
...
// import { stockImg01, stockImg02, stockImg03, stockImg04, stockImg05 } from '../../utils/imagepath';
...
//               <CollapesIcon />
//               <li>
//                 <Link to="#" className="btn btn-secondary w-auto shadow-none" data-bs-toggle="modal" data-bs-target="#send-email">
