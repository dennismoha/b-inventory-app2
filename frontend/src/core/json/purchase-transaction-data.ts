import { company01, company10, company2, company3, company4, company5, company6, company7, company8, company9 } from "../../utils/imagepath";

export const purchaseTransactionData = [
  {
    id: 1,
    invoiceId: "INV001",
    customer: "BrightWave Innovations",
    img: company01,
    email: "michael@example.com",
    createdDate: "12 Sep 2024",
    amount: "$200",
    paymentMethod: "Credit Card",
    status: "Paid",
  },
  {
    id: 2,
    invoiceId: "INV002",
    customer: "Stellar Dynamics",
    img: company2,
    email: "sophie@example.com",
    createdDate: "24 Oct 2024",
    amount: "$600",
    paymentMethod: "Paypal",
    status: "Paid",
  },
  {
    id: 3,
    invoiceId: "INV003",
    customer: "Quantum Nexus",
    img: company3,
    email: "cameron@example.com",
    createdDate: "18 Feb 2024",
    amount: "$200",
    paymentMethod: "Debit Card",
    status: "Paid",
  },
  {
    id: 4,
    invoiceId: "INV004",
    customer: "EcoVision Enterprises",
    img: company4,
    email: "doris@example.com",
    createdDate: "17 Oct 2024",
    amount: "$200",
    paymentMethod: "Paypal",
    status: "Paid",
  },
  {
    id: 5,
    invoiceId: "INV005",
    customer: "Aurora Technologies",
    img: company5,
    email: "thomas@example.com",
    createdDate: "20 Jul 2024",
    amount: "$400",
    paymentMethod: "Credit Card",
    status: "Paid",
  },
  {
    id: 6,
    invoiceId: "INV006",
    customer: "BlueSky Ventures",
    img: company6,
    email: "kathleen@example.com",
    createdDate: "10 Apr 2024",
    amount: "$200",
    paymentMethod: "Paypal",
    status: "Paid",
  },
  {
    id: 7,
    invoiceId: "INV007",
    customer: "TerraFusion Energy",
    img: company7,
    email: "bruce@example.com",
    createdDate: "29 Aug 2024",
    amount: "$4800",
    paymentMethod: "Credit Card",
    status: "Paid",
  },
  {
    id: 8,
    invoiceId: "INV008",
    customer: "UrbanPulse Design",
    img: company8,
    email: "estelle@example.com",
    createdDate: "22 Feb 2024",
    amount: "$50",
    paymentMethod: "Credit Card",
    status: "Unpaid",
  },
  {
    id: 9,
    invoiceId: "INV009",
    customer: "Nimbus Networks",
    img: company9,
    email: "stephen@example.com",
    createdDate: "03 Nov 2024",
    amount: "$600",
    paymentMethod: "Paypal",
    status: "Paid",
  },
  {
    id: 10,
    invoiceId: "INV010",
    customer: "Epicurean Delights",
    img: company10,
    email: "angela@example.com",
    createdDate: "17 Dec 2024",
    amount: "$200",
    paymentMethod: "Credit Card",
    status: "Paid",
  },
];