...
export function formatSharedDate(sharedAt: string | null): string {
  ...
  const diffDays = Math.round((startOfToday.getTime() - startOfShared.getTime()) / msInDay);
...
export function getClinicAdminLoginUrl(workspace: string) {
  ...
  const domain = isLocalhost ? `${workspace}.localhost` : `${workspace}.fusion.dreamstechnologies.com`;
...
export function getCountryFromCallingCode(code: any) {
  ...
  return countries.find((country) => getCountryCallingCode(country) === String(code))?.toLocaleLowerCase();
}

export function getTotalFromArray(values: (string | number)[] | undefined | null): number {
...
export function formatDateToDayMonthYear(dateString: string | null | undefined): string {
...
export function filterByMatchingCodes(dataArray: DataItem[] | null | undefined, codeArray: string[] | null | undefined): DataItem[] {
  ...
  return dataArray.filter((item) => item && typeof item === 'object' && typeof item.code === 'string' && codeSet.has(item.code));
}

// roleRules.ts
export const roleRules: { pattern: RegExp; roles: string[] }[] = [
  { pattern: /^\/pos(\/.*)?$/, roles: ['user', 'admin'] }, // /pos or /pos/anything
  { pattern: /^\/forbidden$/, roles: ['user'] }, // forbidden page
  { pattern: /^\/.*$/, roles: ['admin'] } // default: admin can access anything
];

export const publicPaths = [/^\/signin$/, /^\/logout$/, /^\/register$/];
